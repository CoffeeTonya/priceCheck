...
if selected_item == 'csv検索':
    ...
    price_data = {
        "粗利率": [
            "最安値設定時に粗利率25%以上の場合",
            "最安値設定時に粗利率20%以上25%未満の場合",
            "最安値設定時に粗利率20%未満の場合"
        ],
        "価格設定": [
            "単独で最安値になる価格にする（粗利率23%以上で価格差が大きく出すぎないように設定）",
            "最安値に合わせる",
            """粗利額を確認する  
            ➡ 粗利額が2,000円以上の商品 ・・・ 粗利率17%以上で最安値もしくは、近しい価格に設定  
            ➡ 粗利額が2,000円未満の商品 ・・・ 粗利率20%以上で価格設定"""
        ]
    }

    # 表形式で出力
    for i in range(len(price_data["粗利率"])):
        st.write(f"**粗利率**: {price_data['粗利率'][i]}")
        st.write(f"**価格設定**: {price_data['価格設定'][i]}")
        st.markdown("---")  # 区切り線
