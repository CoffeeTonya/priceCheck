...
if selected_item == '個別検索':
    ...
    orFlag = st.sidebar.radio("複数ワード入力時（0:AND検索 / 1:OR検索）", (0, 1))
    ...
    review = st.sidebar.radio("レビュー（0:すべて / 1:レビューあり）", (0, 1))
    ...
    if not search_keyword:
        ...
        for item in result.get('Items', []):
            tmp_item = {key: item['Item'].get(key, '') for key in item_key}
        ...
        if not df.empty:
            ...
            df['画像'] = df.apply(
                ...
                if isinstance(row["画像"], list) and row["画像"] else '', axis=1)
            ...
            styled_df = df.style.apply(highlight_shop, axis=1).format({'レビュー平均点': "{:.2f}"})

            # CSVファイルとしてデータを出力するボタン
            csv = df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')

            st.download_button(label="CSVファイルとしてダウンロード", data=csv, file_name='楽天市場検索結果.csv', mime='text/csv')
